...
def report_by_region(df):
    ...
    today = str(datetime.today().date()).replace('-', '_')
    filename_out = f'out\\regional\\regional_{today}.xlsx'
    sheet_name='regional'
    ...
    df_out = pd.DataFrame(data=df_out.values, columns=new_columns)
    for c in cols:
        rowcol = getIndexes(df_out, c)
        start_idx, _ = rowcol[0][0], rowcol[0][1]
        start_idx += 3

        rowcol = getIndexes(df_out.iloc[start_idx:], f'Жами, (%):')
        
        mask = df_out[0].iloc[start_idx:] == 'Жами, (%):'
        end_idx = mask[mask].index.values[0]
        end_idx += 1
        worksheet.conditional_format(f'C{start_idx}:Z{end_idx}', {'type':      '3_color_scale'})  
        worksheet.conditional_format(f'C{end_idx+1}:Z{end_idx+1}', {'type':      '3_color_scale',
                                                                    'min_color': '#C5D9F1',
                                                                    'max_color': '#538ED5'})  
        
        worksheet.write(f'A{start_idx-1}', 'Ҳудуд')
        worksheet.write(f'A{start_idx-2}', c)
        merge_format = workbook.add_format({'align': 'center', 'bold': True}).set_font_size(14)
        worksheet.merge_range(f'A{start_idx-2}:{alphabet[merge_columns[c]-1]}{start_idx-2}', c, merge_format)
        border_fmt = workbook.add_format({'bottom':1, 'top':1, 'left':1, 'right':1})
        worksheet.conditional_format(f'A{start_idx-2}:{alphabet[merge_columns[c]-1]}{end_idx+2}', {'type': 'no_errors', 'format': border_fmt, 'bold': True})
    ...
    break_col = max([e[1] for e in merge_columns.items()])
    worksheet.set_h_pagebreaks([break_col])
